...
public class InternalDynamoDBSigner {
     ...
     */
    static InternalDynamoDBSigner getInstance(String signingAlgorithm, SecureRandom rnd) {
        InternalDynamoDBSigner result = cache.get(signingAlgorithm);
        if (result == null) {
            result = new InternalDynamoDBSigner(signingAlgorithm, rnd);
     ...
     *            algorithm is fully specified by the key.
     */
    private InternalDynamoDBSigner(String signingAlgorithm, SecureRandom rnd) {
        if (rnd == null) {
            rnd = Utils.getRng();
        }
        this.rnd = rnd;
